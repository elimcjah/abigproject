...
let writeToFile = function(data, fname) {
...
let loadGoogle = function(callback) {
    let request = require('request');
    request('http://google.com', function(error, response, body) {
        /*
        ...
        */
        ...
        let fname = 'google' + ((!error) ? response.statusCode : 'err') + 
            '.html';
        callback(body, fname);
    });
}
...
let promise = loadGoogle(); // it returns a promise, 'member?
