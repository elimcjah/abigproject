...
class SpiritTests {
    ...
    configureJasmine() {
        ...
        let completeFn = (passed) => {
            ...
            let visit = require('universal-analytics')(
                this.configs['tests']['ua-id'], 
                this.configs['tests']['ua-uuid']);
            let scope = this.configs['tests']['ua-scope'].length ? 
                `(${this.configs['tests']['ua-scope']}) ` : '';
            if (passed) {
                visit.event(`${scope}Tests`, 'Passed', 
                    this.configs['tests']['ua-uuid']);
            } else {
                visit.event(`${scope}Tests`, 'Failed', 
                    this.configs['tests']['ua-uuid']);
            }
            for (let suite in this.suites) {
                for (let spec of this.suites[suite].specs) {
     ...
     * @param {Array} fileFilter (Optional) If provided, only executes tests
     * in the provided file(s).
     * @return {Promise} A promise that resolves if the tests pass.
     */
    execute(silent, fileFilter) {
        let promise;
        if (this.executing) {
            return;
        }
        ...
        fileFilter = require('spiritfiles/spiritfiles.class.js')
            .toFilenames(fileFilter);
        this.testFiles = this.testFiles.filter((file) => {
            if (fileFilter.indexOf(require
                ('spiritfiles/spiritfiles.class.js')
                .toFilename(file)) > -1) {
                return true;
            }
            return false;
    ...
    readTests() {
        ...
        for (folder of folders) {
            if (!this.fs.existsSync(folder) &&
                this.fs.existsSync(process.cwd() + '/node_modules/' + folder)) {
                folder = process.cwd() + '/node_modules/' + folder;
            }
            if (!this.fs.existsSync(folder)) {
                continue;
            }
        ...
        if (this.configs.hasOwnProperty('tests')) {
            if (this.configs['tests'].hasOwnProperty('files')) {
                for (let idx in this.configs['tests']['files']) {
                    let file = this.configs.tests.files[idx];
                    if (!this.fs.existsSync(file) &&
                        this.fs.existsSync('./node_modules/' + file)) {
                            file = './node_modules' + file;
                            this.configs.tests.files[idx] = file;
                        }
                }
