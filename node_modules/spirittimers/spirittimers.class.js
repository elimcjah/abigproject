...
class SpiritTimers {
     ...
     *     triggered.
     * @param {Number} interval The amount of milisecons between timer calls OR
     *     a string time (e.g. 1w2d5h).
     * @param {Object} options The options object.
     */
    addTimer(callbackFn, interval, options) {
        let timer;
        if (typeof callbackFn != 'function') {
            console.log(this.chalk.red('SpiritTimers Error: ') + 'attempted' + 
                ' to create a timer without a function.');
        }
        timer = new this.spiritTimer(callbackFn, interval, options);
     ...
     * @param {Object} options The options object.
     */
    addTimerAndExecute(callbackFn, interval, options) {
        let timer;
        timer = this.addTimer(callbackFn, interval, options);
        timer.execute();
        return timer;
    }
     ...
     * does not exist.
     */
    static getTimerFiles() {
        if (!require('fs').existsSync(process.cwd() + '/' + 'timers')) {
            return [];
        }
        return require('spiritfiles').readFiles('timers', SpiritTimers
            .readTimerFiles, 1000 * 60);
    }
     ...
     */
    static readTimerFiles() {
        let file, files, timer;
        files = SpiritTimers.getTimerFiles();
        if (!files) {
            return false;
        }
        for (file of files) {
            ...
            require(__dirname + '/spirittimer.class.js').fromFile(file);
        }
    }
